...
<head>
    ...
    <style>
        :root { --bg-dark: #050505; --bg-gradient: linear-gradient(180deg, #000000 0%, #111 100%); --accent: #FFD700; --card-bg: #1e1e1e; --text-main: #f0f0f0; }
        body { background: var(--bg-dark); background-image: var(--bg-gradient); background-attachment: fixed; padding-bottom: 80px; font-family: 'Segoe UI', Roboto, sans-serif; color: var(--text-main); overflow-x: hidden; user-select: none; }
        [data-bs-theme="light"] body { --bg-dark: #f0f2f5; --bg-gradient: linear-gradient(180deg, #ffffff 0%, #e9ecef 100%); --card-bg: #ffffff; --text-main: #333333; color: #333; }
        ...
        .hero-header { background: linear-gradient(to bottom, #000 0%, #222 100%); padding: 20px; text-align: center; border-bottom: 3px solid var(--accent); border-radius: 0 0 25px 25px; margin-bottom: 25px; position: relative; }
        .theme-toggle { position: absolute; top: 15px; right: 15px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: var(--accent); width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 50; backdrop-filter: blur(4px); }
        .lion-img { max-width: 140px; height: auto; display: block; margin: 0 auto 5px auto; filter: drop-shadow(0 0 8px rgba(255, 215, 0, 0.3)); }
        .brand-title { font-family: 'Arial Black', sans-serif; font-size: 2rem; color: var(--accent); text-transform: uppercase; letter-spacing: 2px; margin: 0; text-shadow: 2px 2px 0 #000; }
        ...
        .form-control-lg { background: rgba(45, 45, 45, 0.9); border: 1px solid #555; color: white; border-radius: 12px; padding: 12px 20px; }
        ...
        .alphabet-bar { display: flex; flex-wrap: wrap; justify-content: center; gap: 6px; padding: 15px 10px; }
        .letter-btn { flex: 0 0 auto; width: 42px; height: 38px; border-radius: 8px; border: 1px solid #333; background: #1a1a1a; color: #888; font-weight: bold; font-size: 0.9rem; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; transition: 0.2s; line-height: 1; }
        .letter-btn-wide { width: auto; padding: 0 15px; }
        .letter-btn.active { background: var(--accent); color: #000; font-weight: 900; box-shadow: 0 0 10px rgba(255,215,0,0.4); }
        ...
        .code-btn { background: #0d6efd; color: white; padding: 8px 10px; border-radius: 6px; font-weight: 900; font-size: 1.1rem; min-width: 75px; }
        ...
        .fab-download { position: fixed; bottom: 25px; right: 25px; background: #198754; color: white; width: 60px; height: 60px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; box-shadow: 0 5px 15px rgba(0,0,0,0.5); text-decoration: none; z-index: 1000; }
    </style>
</head>
<body>
<div id="app">
    ...
    <div class="container pb-5">
        <div v-for="m in listaPaginada" :key="m.c" class="card-music">
            ...
            <div class="card-actions">
                ...
                <a :href="'https://www.google.com/search?q=letra+' + m.a + '+' + m.m" target="_blank" class="bi bi-search btn-icon" title="Pesquisar Letra no Google"></a>
